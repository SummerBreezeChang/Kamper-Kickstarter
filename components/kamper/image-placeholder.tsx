...
export function ImagePlaceholder({ title, note, className, showFrame = true }: ImagePlaceholderProps) {
  return (
    <div
      className={`relative flex h-full w-full items-center justify-center overflow-hidden ${
        showFrame ? "rounded-2xl border border-foreground/25 bg-foreground/5" : ""
      } ${className ?? ""}`}
    >
      {showFrame ? (
        <>
